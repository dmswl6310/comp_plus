...
src/
├─ components/
│  ├─ common/                 # Header, Footer, CodeTabs, CodeBlock 등 공통 UI
...
```txt
src/data/components/[component-name]/
├─ CustomXxx.tsx     # 실제 컴포넌트 구현
├─ examples.tsx      # 미리보기용 example props 배열
├─ meta.ts           # 이름, 설명, 태그, 썸네일, propControls 등 메타데이터
...
- 각 컴포넌트 폴더의 `default export`를 자동 수집
...
**컴포넌트 폴더의 `index.ts`만 잘 작성하면 registry에 자동으로 포함되는 방향**으로 정리된 상태입니다.
...
3. `propControls`를 기준으로 입력 UI를 생성
4. 값이 바뀌면 `propsState`를 갱신
5. 변경된 props로 컴포넌트를 다시 렌더링
...
### 1) 새 폴더 생성
```txt
src/data/components/my-component/
```
